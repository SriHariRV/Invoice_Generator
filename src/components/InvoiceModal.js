        ...
        <Modal show={this.props.showModal} onHide={this.props.closeModal} size="lg" centered>
          <div id="invoiceCapture">
            <div className="d-flex flex-row justify-content-between align-items-start bg-light w-100 p-4">
              <div className="w-100">
                <h4 className="fw-bold my-2">{this.props.info.billFromCompany || 'Company Name'}</h4> {/* Updated to show Bill From Company */}
            ...
            <div className="p-4">
              <Row className="mb-4">
                <Col md={4}>
                  ...
                  <div>{this.props.info.billFromCompany || ''}</div> {/* Updated to show Bill From Company */}
                  <div>{this.props.info.billFrom || ''}</div>
                  <div>{this.props.info.billFromAddress || ''}</div>
                  <div>{this.props.info.billFromEmail || ''}</div>
                  <div>{this.props.info.billFromPhone || ''}</div>
                  <div>{this.props.info.billFromDesignation || ''}</div>
                </Col>
                <Col md={4}>
                  <div className="fw-bold">Billed To:</div>
                  <div>{this.props.info.billToCompany || ''}</div> {/* Updated to show Bill To Company */}
                  <div>{this.props.info.billTo || ''}</div>
                  <div>{this.props.info.billToAddress || ''}</div>
                  <div>{this.props.info.billToEmail || ''}</div>
                  <div>{this.props.info.billToPhone || ''}</div>
                  <div>{this.props.info.billToDesignation || ''}</div>
                </Col>
                <Col md={4}>
